...
{
	log("FacebookMgr::OnGetResponse %d [%s]", nIndex, pszStr ? pszStr : "NULL");
...
void FacebookMgr::ProcessJsonResponse( int nIndex, const char *pszStr )
{
	...
	case FBACTION_REQ_FRIENDINFO:
		{
			auto c = Json_getItem(pJsonValue, "data");
			m_vecFriends.clear();

		    Json *next;
            while(c){
				next = c->next;
				const char *pszId = Json_getString(c, "id", "0");
				FB_USERINFO info;
				sscanf(pszId, "%lld", &info.m_i64Id);
				info.m_strName = Json_getString(c, "name", "");
...
void FacebookMgr::OnDownloadCallback(HttpClient *sender, HttpResponse *response)
{
	if (NULL == response) {
